...
-- The following code is used to test the disabling of triggers

CREATE OR REPLACE FUNCTION insert_test1()
  RETURNS trigger AS
  $BODY$
    BEGIN
      NEW.name = NEW.name || ' (modified)';
      RETURN NEW;
    END;
  $BODY$
  LANGUAGE plpgsql VOLATILE SECURITY DEFINER;

CREATE TRIGGER insert_test1_trigger
   BEFORE INSERT ON test1_tbl
   FOR EACH ROW
   EXECUTE PROCEDURE insert_test1();
