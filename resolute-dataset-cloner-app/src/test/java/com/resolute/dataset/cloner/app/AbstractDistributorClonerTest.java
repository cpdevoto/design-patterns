...
public abstract class AbstractDistributorClonerTest extends AbstractDatabaseTest {
  ...
  void test_execute() {
    ...
    DistributorCloner cloner = DistributorCloner.builder(env)
        .withDistributorId(2)
        .withBeforeAllListener(() -> System.out.println("Starting distributor clone operation..."))
        .withAfterAllListener((elapsed) -> System.out
            .println("Finished distributor clone operation in " + ElapsedTimeUtils.format(elapsed)))
        .build();

    int prefix = cloner.getTableNamePrefix();

    assertThat(cloner.getOrphanedSuperclassNodes())
        .as("DistributorCloner contains the following orphaned superclass nodes: "
    ...
    try {
      ...
      new DistributorCloneAssertions(this, acTags).assertStateAfterClone(prefix);
