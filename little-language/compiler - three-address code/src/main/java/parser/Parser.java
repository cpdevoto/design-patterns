...
public class Parser {
  ...
  private void decls() throws IOException {
    while (look.tag == Tag.BASIC) {  // Type -> type ID ;
      Type p = type();
      Token tok = look;
      match(Tag.ID);
      match(';');
  ...
  private Stmt stmt() throws IOException {
    Expr x;
    Stmt s1, s2;  
    Stmt savedStmt;  // save enclosing loops for break 
    switch (look.tag) {
      case ';':
        move();
        return Stmt.NULL;
  ...
  private Stmt assign() throws IOException {
    ...
    if (look.tag == '=') {  // S -> id = E ; Normal assignment
      move();
      stmt = new Set(id, bool());
    } else {                // S -> L = E ; Array element assignment
      Access x = offset(id);
      match('=');
  ...
  private Access offset(Id a) throws IOException { // I -> [E] | [E] I
    Expr i; // the index
    Expr w; // the number of bytes required for each element
    Expr t1, t2;
    Expr loc;
    ...
    t1 = new Arith(new Token('*'), i, w);  // the byte offset from the start of the array
    ...
    while (look.tag == '[') { // multi-dimensional I -> [ E ] I
      ...
      w = new Constant(type.width);
      t1 = new Arith(new Token('*'), i, w);  // the byte offset from the start of the array
      t2 = new Arith(new Token('+'), loc, t1);  // sum up the computed offsets
      loc = t2;
    }
