...
-- these tables are used to test whether we can clone records with compound foreign keys

CREATE TABLE test11_tbl (
  id1 SERIAL,
  id2 SERIAL,
  name CHARACTER VARYING NOT NULL UNIQUE CHECK (name <> ''),
...
CREATE TABLE test12_tbl (
  test11_id1 INTEGER NOT NULL,
  test11_id2 INTEGER NOT NULL,
  name CHARACTER VARYING NOT NULL UNIQUE CHECK (name <> ''),
  PRIMARY KEY (test11_id1, test11_id2),
  CONSTRAINT test12_id_fkey FOREIGN KEY (test11_id1, test11_id2) 
          REFERENCES test11_tbl(id1, id2) ON DELETE CASCADE
