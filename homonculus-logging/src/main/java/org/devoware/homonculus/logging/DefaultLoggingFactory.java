...
import org.devoware.homonculus.logging.async.AsyncLoggingEventAppenderFactory;
import org.devoware.homonculus.logging.filter.LevelFilterFactory;
import org.devoware.homonculus.logging.filter.ThresholdLevelFilterFactory;
import org.devoware.homonculus.logging.layout.HomonculusLayoutFactory;
...
public class DefaultLoggingFactory implements LoggingFactory {
  ...
  public void configure(MetricRegistry metricRegistry, String name) {
    ...
    final LayoutFactory<ILoggingEvent> layoutFactory = new HomonculusLayoutFactory();

    for (AppenderFactory<ILoggingEvent> output : appenders) {
      root.addAppender(output.build(loggerContext, name, layoutFactory, levelFilterFactory,
  ...
  public void stop() {
    ...
    try {
      ...
      final ArrayList<Appender<ILoggingEvent>> appenders =
          Lists.newArrayList(logger.iteratorForAppenders());
  ...
  public void reset() {
    ...
    try {
      ...
      final HomonculusLayout formatter = new HomonculusLayout(loggerContext, TimeZone.getDefault());
      formatter.start();
  ...
  private Logger configureLoggers(String name) {
    ...
    for (Map.Entry<String, JsonNode> entry : loggers.entrySet()) {
      ...
      if (jsonNode.isTextual()) {
        ...
        try {
          configuration = ObjectMappers.create().treeToValue(jsonNode, LoggerConfiguration.class);
        ...
        for (AppenderFactory<ILoggingEvent> appender : configuration.getAppenders()) {
          logger.addAppender(appender.build(loggerContext, name, layoutFactory, levelFilterFactory,
              asyncAppenderFactory));
        }
      } else {
  ...
  static Level toLevel(String text) {
