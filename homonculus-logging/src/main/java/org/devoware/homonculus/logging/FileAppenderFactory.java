...
import org.devoware.homonculus.validators.validation.MinSize;
 ...
 * <td>The filename pattern for archived files. If {@code maxFileSize} is specified, rollover is
 * size-based, and the pattern must contain {@code %i} for an integer index of the archived file.
 * Otherwise rollover is date-based, and the pattern must contain {@code %d}, which is replaced with
 * the date in {@code yyyy-MM-dd} form. If the pattern ends with {@code .gz} or {@code .zip}, files
 * will be compressed as they are archived.</td>
 * </tr>
 * <tr>
 * <td>{@code archivedFileCount}</td>
 * <td>{@code 5}</td>
 * <td>The number of archived files to keep. Must be greater than or equal to {@code 0}. Zero is a
 * special value signifying to keep infinite logs (use with caution)</td>
 * </tr>
 * <tr>
 * <td>{@code maxFileSize}</td>
 * <td>(unlimited)</td>
 * <td>The maximum size of the currently active file before a rollover is triggered. The value can
 * be expressed in bytes, kilobytes, megabytes, gigabytes, and terabytes by appending B, K, MB, GB,
 * or TB to the numeric value. Examples include 100MB, 1GB, 1TB. Sizes can also be spelled out, such
 * as 100 megabytes, 1 gigabyte, 1 terabyte.</td>
 * </tr>
 * <tr>
 * <td>{@code timeZone}</td>
 * <td>{@code UTC}</td>
 ...
 * this from the default of 8KB to 256KB is reported to significantly reduce thread contention.</td>
 * </tr>
 * <tr>
 * <td>{@code immediateFlush}</td>
 * <td>{@code true}</td>
 * <td>If set to true, log events will be immediately flushed to disk. Immediate flushing is safer,
 * but it degrades logging throughput. See
 * <a href="https://logback.qos.ch/manual/appenders.html#immediateFlush">the Logback
