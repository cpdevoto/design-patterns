...
import org.devoware.homonculus.config.validation.ValidationMethod;
 ...
 * <td>{@code false}</td>
 * <td>Remove abandoned connections if they exceed the {@code removeAbandonedTimeout}. If set to
 * {@code true} a connection is considered abandoned and eligible for removal if it has been in use
 * longer than the {@code removeAbandonedTimeout} and the condition for
 * {@code abandonWhenPercentageFull} is met.</td>
 * </tr>
 * <tr>
 * <td>{@code removeAbandonedTimeout}</td>
 * <td>60 seconds</td>
 * <td>The time before a database connection can be considered abandoned.</td>
 * </tr>
 * <tr>
 * <td>{@code abandonWhenPercentageFull}</td>
 * <td>0</td>
 * <td>Connections that have been abandoned (timed out) won't get closed and reported up unless the
 * number of connections in use are above the percentage defined by
 * {@code abandonWhenPercentageFull}. The value should be between 0-100.</td>
 * </tr>
 * <tr>
 ...
 * even when not used.</td>
 * </tr>
 * <tr>
 * <td>{@code commitOnReturn}</td>
 * <td>{@code false}</td>
 * <td>Set to true if you want the connection pool to commit any pending transaction when a
 * connection is returned.</td>
 * </tr>
 * <tr>
 * <td>{@code rollbackOnReturn}</td>
 * <td>{@code false}</td>
 * <td>Set to true if you want the connection pool to rollback any pending transaction when a
 * connection is returned.</td>
 * </tr>
 * <tr>
 ...
 * <td>The default transaction isolation to use for the connections. Can be one of {@code none},
 * {@code default}, {@code read-uncommitted}, {@code read-committed}, {@code repeatable-read}, or
 * {@code serializable}.</td>
 * </tr>
 * <tr>
 * <td>{@code useFairQueue}</td>
 * <td>{@code true}</td>
 * <td>If {@code true}, calls to {@code getConnection} are handled in a FIFO manner.</td>
 * </tr>
 * <tr>
 * <td>{@code initialSize}</td>
 * <td>10</td>
 * <td>The initial size of the connection pool. May be zero, which will allow you to start the
 * connection pool without requiring the DB to be up. In the latter case the {@link #minSize} must
 * also be set to zero.</td>
 * </tr>
 * <tr>
 * <td>{@code minSize}</td>
 * <td>10</td>
 * <td>The minimum size of the connection pool.</td>
 * </tr>
 * <tr>
 * <td>{@code maxSize}</td>
 * <td>100</td>
 * <td>The maximum size of the connection pool.</td>
 * </tr>
 * <tr>
 * <td>{@code initializationQuery}</td>
 * <td>none</td>
 * <td>A custom query to be run when a connection is first created.</td>
 * </tr>
 * <tr>
 * <td>{@code logAbandonedConnections}</td>
 * <td>{@code false}</td>
 * <td>If {@code true}, logs stack traces of abandoned connections.</td>
 * </tr>
 * <tr>
 * <td>{@code logValidationErrors}</td>
 * <td>{@code false}</td>
 * <td>If {@code true}, logs errors when connections fail validation.</td>
 * </tr>
 * <tr>
 * <td>{@code maxConnectionAge}</td>
 * <td>none</td>
 * <td>If set, connections which have been open for longer than {@code maxConnectionAge} are closed
 * when returned.</td>
 * </tr>
 * <tr>
 * <td>{@code maxWaitForConnection}</td>
 * <td>30 seconds</td>
 * <td>If a request for a connection is blocked for longer than this period, an exception will be
 * thrown.</td>
 * </tr>
 * <tr>
 * <td>{@code minIdleTime}</td>
 * <td>1 minute</td>
 * <td>The minimum amount of time an connection must sit idle in the pool before it is eligible for
 * eviction.</td>
 * </tr>
 * <tr>
 * <td>{@code validationQuery}</td>
 * <td>{@code SELECT 1}</td>
 * <td>The SQL query that will be used to validate connections from this pool before returning them
 * to the caller or pool. If specified, this query does not have to return any data, it just can't
 * throw a SQLException.</td>
 * </tr>
 * <tr>
 * <td>{@code validationQueryTimeout}</td>
 * <td>none</td>
 * <td>The timeout before a connection validation queries fail.</td>
 * </tr>
 * <tr>
 * <td>{@code checkConnectionWhileIdle}</td>
 * <td>{@code true}</td>
 * <td>Set to true if query validation should take place while the connection is idle.</td>
 * </tr>
 * <tr>
 * <td>{@code checkConnectionOnBorrow}</td>
 * <td>{@code false}</td>
 * <td>Whether or not connections will be validated before being borrowed from the pool. If the
 * connection fails to validate, it will be dropped from the pool, and another will be borrowed.
 * </td>
 * </tr>
 * <tr>
 * <td>{@code checkConnectionOnConnect}</td>
 * <td>{@code false}</td>
 * <td>Whether or not connections will be validated before being added to the pool. If the
 * connection fails to validate, it won't be added to the pool.</td>
 * </tr>
 * <tr>
 * <td>{@code checkConnectionOnReturn}</td>
 * <td>{@code false}</td>
 * <td>Whether or not connections will be validated after being returned to the pool. If the
 * connection fails to validate, it will be dropped from the pool.</td>
 * </tr>
 * <tr>
 * <td>{@code autoCommentsEnabled}</td>
 * <td>{@code true}</td>
 * <td>Whether or not ORMs should automatically add comments.</td>
 * </tr>
 * <tr>
 * <td>{@code evictionInterval}</td>
 * <td>5 seconds</td>
 * <td>The amount of time to sleep between runs of the idle connection validation, abandoned cleaner
 * and idle pool resizing.</td>
 * </tr>
 * <tr>
 * <td>{@code validationInterval}</td>
 * <td>30 seconds</td>
 * <td>To avoid excess validation, only run validation once every interval.</td>
 * </tr>
 * <tr>
 * <td>{@code validatorClassName}</td>
 * <td>(none)</td>
 * <td>Name of a class of a custom {@link org.apache.tomcat.jdbc.pool.Validator} implementation,
 * which will be used for validating connections.</td>
 * </tr>
 * </table>
 */
public class DataSourceFactory implements PooledDataSourceFactory {
  public enum TransactionIsolation {
    NONE(Connection.TRANSACTION_NONE), DEFAULT(
        org.apache.tomcat.jdbc.pool.DataSourceFactory.UNKNOWN_TRANSACTIONISOLATION), READ_UNCOMMITTED(
            Connection.TRANSACTION_READ_UNCOMMITTED), READ_COMMITTED(
                Connection.TRANSACTION_READ_COMMITTED), REPEATABLE_READ(
                    Connection.TRANSACTION_REPEATABLE_READ), SERIALIZABLE(
  ...
  public ManagedDataSource build(String name) {
    return build(new MetricRegistry(), name);
  }
